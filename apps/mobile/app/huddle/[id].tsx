...
import { AudioSession, LiveKitRoom, VideoTrack, useLocalParticipant, useParticipants, useTracks } from "@livekit/react-native";
...
function HuddleStage() {
  ...
  const [updating, setUpdating] = useState<"camera" | "microphone" | null>(null);
  const [controlError, setControlError] = useState(false);
  async function toggle(kind: "camera" | "microphone") {
    try { setUpdating(kind); setControlError(false); if (kind === "camera") await localParticipant.setCameraEnabled(!isCameraEnabled); else await localParticipant.setMicrophoneEnabled(!isMicrophoneEnabled); }
  ...
  return <View style={styles.stage}>
    ...
    <View style={styles.controls}><Pressable onPress={() => void toggle("microphone")} disabled={updating !== null} style={[styles.control, !isMicrophoneEnabled && styles.controlOff]}><Text style={styles.controlText}>{isMicrophoneEnabled ? "マイク ON" : "マイク OFF"}</Text></Pressable><Pressable onPress={() => void toggle("camera")} disabled={updating !== null} style={[styles.control, !isCameraEnabled && styles.controlOff]}><Text style={styles.controlText}>{isCameraEnabled ? "カメラ ON" : "カメラ OFF"}</Text></Pressable></View>
    {(controlError || lastMicrophoneError || lastCameraError) && <Text style={styles.mediaError}>端末のマイクまたはカメラを利用できません。設定から権限を確認してください。</Text>}
  </View>;
}
...
const styles = StyleSheet.create({ screen:{flex:1,padding:20,backgroundColor:"#171717"},body:{flex:1,justifyContent:"center",gap:16},eyebrow:{fontSize:11,color:"#AFAFA8",fontWeight:"600"},title:{fontSize:27,lineHeight:38,color:"#fff",fontWeight:"500"},stage:{height:220,borderRadius:12,overflow:"hidden",backgroundColor:"#292929",gap:8},video:{flex:1,backgroundColor:"#292929"},videoTrack:{flex:1},name:{position:"absolute",left:10,bottom:10,color:"#fff",fontSize:12,fontWeight:"600",backgroundColor:"#0008",paddingHorizontal:7,paddingVertical:4,borderRadius:6},waiting:{flex:1,justifyContent:"center",alignItems:"center",padding:24,gap:6},waitingTitle:{color:"#fff",fontSize:16,fontWeight:"600"},waitingCopy:{color:"#B8B8B4",fontSize:12,lineHeight:19,textAlign:"center"},participantCount:{position:"absolute",right:10,top:10,color:"#ddd",fontSize:11,backgroundColor:"#0008",paddingHorizontal:7,paddingVertical:4,borderRadius:6},controls:{position:"absolute",left:10,bottom:10,flexDirection:"row",gap:8},control:{backgroundColor:"#000B",paddingHorizontal:9,paddingVertical:7,borderRadius:7},controlOff:{backgroundColor:"#7B3131"},controlText:{color:"#fff",fontSize:11,fontWeight:"600"},mediaError:{position:"absolute",left:10,right:10,bottom:46,color:"#FFD6D6",fontSize:10,lineHeight:14},record:{padding:14,borderRadius:10,backgroundColor:"#3B2020",flexDirection:"row",gap:9,alignItems:"flex-start"},dot:{height:9,width:9,borderRadius:5,marginTop:5,backgroundColor:"#E35555"},recordText:{flex:1,fontSize:12,lineHeight:20,color:"#FFD6D6"},hint:{fontSize:12,lineHeight:20,color:"#B8B8B4"},end:{marginTop:8,padding:15,alignItems:"center",borderRadius:8,borderWidth:1,borderColor:"#777"},endDisabled:{opacity:.55},endText:{color:"#fff",fontSize:13,fontWeight:"600"} });

...
import { createHuddle, joinHuddle } from "../src/api";

export default function Huddle() {
  const [notice, setNotice] = useState("AIがSarahとの10分ハドルを準備できます。");
  async function start() { try { const { huddle } = await createHuddle(); setNotice(huddle.recordingDisclosure); await joinHuddle(huddle.id); router.push({ pathname: "/huddle/[id]", params: { id: huddle.id } }); } catch (error) { setNotice(error instanceof Error ? error.message : "接続できませんでした。APIを起動して、もう一度試してください。"); } }

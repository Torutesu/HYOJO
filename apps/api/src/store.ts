...
function decodeDocument<T>(document: unknown): T {
  if (typeof document === "string") return JSON.parse(document) as T;
...
class PostgresStore implements HyojoStore {
  ...
  private async getDocument<T>(table: "space_policies" | "huddles" | "huddle_memories" | "huddle_transcripts", key: string): Promise<T | undefined> {
    ...
    return rows[0] ? decodeDocument<T>(rows[0].document) : undefined;
  }
  ...
  async listHuddles() { const rows = await this.sql`select document from huddles`; return rows.map((row) => decodeDocument<Huddle>(row.document)); }
  ...
  async listAuditEvents() { const rows = await this.sql`select document from audit_events order by created_at desc`; return rows.map((row) => decodeDocument<AuditEvent>(row.document)); }
